...
<body class="hold-transition sidebar-mini">
		...
		<div class="content-wrapper">
			...
			<section class="content">
				<div class="container-fluid">
					...
					<div class="modal fade" id="modalInputPackingDetail" role="dialog">
						<div class="modal-dialog modal-lg">
							<div class="modal-content">
								<div class="modal-header">
									<h4 class="modal-title">Detail Input Packing</h4>
									<button type="button" class="close" data-dismiss="modal" aria-label="Close">
										<span aria-hidden="true">&times</span>
									</button>
								</div>
								<div class="modal-body">
									<table id="tableInputPackingDetail" class="table table-striped table-bordered" cellpadding="5" style="width: 100%">
										<thead>
											<tr>
												<th>Size</th>
												<th>Qty</th>
	...
	<script>
		$(document).ready(function() {
			const regBarcodeBundle = /(cp|bw|cu|as|pa)+_g\d{1}-\d{2}\w{1}\d{3}\w{1}-?(\d{1}|)-\d{4}/i; //regex untuk barcode Sewing
			...
			// 	$.ajax({
			// 		type: 'POST',
			// 		url: '<//?php echo site_url("InputPacking/ajax_get_by_orc"); ?>',
			// 		data: {
			// 			"orc": d['orc']
			// 		},
			// 		dataType: 'json'
			...
			// 		var htmlDetailTable = "";
			// 		// var htmlDetail = "";
			// 		htmlDetailTable = 'table id="inputPackingDetail" class="table table-striped table-bordered" cellpadding="5" cellspacing="0" border="0" style="padding-left:50px;">';
			...
			$('#kode_barcode').keypress(function(e) {
				if (e.keyCode == 13) {
					...
					if (!regBarcodeBundle.test(barcode)) {
						...
						$.ajax({
							type: "GET",
							url: '<?php echo site_url("InputPacking/ajax_check_by_barcode"); ?>/' + kode[1],
							dataType: 'json',
						}).done(function(rst) {
							console.log('rst: ', rst);
							if (rst.outputSewingDetail == null) {
								Swal.fire({
									type: 'warning',
									title: 'Warning',
							...
							} else if (rst.inputPacking == null && rst.outputSewingDetail != null) {
								qty = rst.outputSewingDetail.qty;
								showBarcodeDetail()
							} else if (rst.inputPacking != null && rst.outputSewingDetail != null) {
								if (parseInt(rst.outputSewingDetail.qty) == parseInt(rst.inputPacking.actual_qty)) {
									Swal.fire({
										type: 'warning',
										title: 'Warning',
										html: '<strong>Barcode sudah selesai di scan!!</strong>',
										showConfirmButton: false,
										timer: 1000,
										onAfterClose: () => {
											$('#kode_barcode').val('');
									...
									let qtyFromOutputSewingDetail = parseInt(rst.outputSewingDetail.qty);
									let qtyFromInputPacking = parseInt(rst.inputPacking.actual_qty);
									qty = qtyFromOutputSewingDetail - qtyFromInputPacking;
									console.log('qtyFromOutputSewingDetail: ', qtyFromOutputSewingDetail);
									console.log('qtyFromInputPacking: ', qtyFromInputPacking);
									console.log('qty: ', qty);
									showBarcodeDetail();
								}
							}
			...
			function showDetail(orc) {
				tableDetail = $('#tableInputPackingDetail').DataTable({
					destroy: true,
					ajax: {
						type: 'POST',
						url: '<?= site_url("InputPacking/ajax_get_by_orc"); ?>',
						data: {
							orc: orc
						},
						dataType: 'json'
				...
				$('#modalInputPackingDetail').modal('show');

			}

			$('#modalInputPackingDetail').on('hidden.bs.modal', function(e) {
