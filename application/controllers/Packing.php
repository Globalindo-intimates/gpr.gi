...
class Packing extends CI_Controller
{
	...
	private function _print_barcode_packing1($orc, $rst)
	{
		...
		for ($p = 1; $p <= $pages; $p++) {
			...
			foreach ($rows as $r) {
				...
				$pdf->write1DBarcode($r->barcode, 'C128', $x, $y - 3.5, 45, 11, 0.4, $style, 'N');
	...
	private function _print_barcode_packing($orc, $rst)
	{
		...
		for ($p = 1; $p <= $pages; $p++) {
			...
			foreach ($rows as $r) {
				...
				//barcode
				// $pdf->write1DBarcode($r->barcode, 'C128', $x, $y - 3.5, 50, 12, 0.4, $style, 'T');
				$pdf->write1DBarcode($r->barcode, 'C128', $x + 1.5, $y - 0.2, 47, 12, 0.4, $style, 'T');
	...
	private function _print_barcode_packing2($orc, $rst)
	{
		...
		// $width = $pdf->pixelsToUnits(793.701); //folio width
		// $height = $pdf->pixelsToUnits(1247.244); //folio height
		$width = 216;
		$height = 330;
		...
		$counterRows = 1;
		// $labelsPerPage = 30;
		// $pages = (int)round(count($rst)/$labelsPerPage);
		...
		for ($i = 0; $i <= count($rst) - 1; $i++) {
			...
			//barcode
			$pdf->SetFont('Helvetica', 'B', 10);
			$pdf->write1DBarcode($rst[$i]->barcode, 'C128', $x, $y, $labelWidth, $labelHeight, 0.4, $barcodeStyle, 'L');

			//orc, size, qty, box capacity
			$pdf->SetXY($x + 1, $y + $labelHeight);
			$pdf->SetFont('Helvetica', 'B', 12);
			$pdf->Cell($labelWidth, 7, $orc . "  " . $rst[$i]->size . " " . $rst[$i]->qty . " " . $rst[$i]->box_capacity . " ", 0, 0, 'L', FALSE, '', 0, FALSE, 'C', 'B');

			//nmr box
			$pdf->SetXY($x, $y + $labelHeight);

			// $totBox = $rst[$i]->total_box;
			// if ($totBox != $rst[$i - 1]->total_box) {
			// 	$totBox = $totBox + $rst[$i]->total_box;
			// }

			// if ($i == 0) {
			// 	$pdf->Cell($labelWidth, 5, $rst[$i]->no_box . " - " . $rst[$i]->total_box, 0, 0, 'R', FALSE, '', 0, FALSE, 'C', 'B');
			// } elseif ($i > 0) {
			// 	$totBoxPrev = $rst[$i - 1]->total_box;
			// 	if ($totBoxPrev == $rst[$i]->total_box) {
			// 		$totBox = $rst[$i]->total_box;
			// 	} else {
			// 		$totBox = $rst[$i]->total_box + $rst[$i - 1]->total_box;
			// 	}
			// 	$pdf->Cell($labelWidth, 5, $rst[$i]->no_box . " - " . $totBox, 0, 0, 'R', FALSE, '', 0, FALSE, 'C', 'B');
			// }
			$pdf->SetFont('Helvetica', 'B', 12);
